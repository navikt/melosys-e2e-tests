...
test.describe('EU/EØS - Arbeid i flere land (artikkel 13.1)', () => {
  test('skal fullføre arbeid i flere land-arbeidsflyt med POM', async ({ page }) => {
    // Oppsett
    ...
    console.log('✅ EU/EØS "Arbeid i flere land" arbeidsflyt fullført med POM');
  });

  test('skal fullføre arbeid i flere land-arbeidsflyt steg-for-steg', async ({ page }) => {
    // Oppsett
    const auth = new AuthHelper(page);
    await auth.login();
    ...
    // Fullfør behandling steg-for-steg
    await behandling.klikkBekreftOgFortsett(); // Initial bekreftelse

    // Steg 1: Velg hjemland
    await behandling.velgHjemlandOgFortsett('Norge');

    // Steg 2: Velg arbeidsgiver
    await behandling.velgArbeidsgiverOgFortsett('Ståles Stål AS');

    // Steg 3: Bekreft arbeid i flere land
    await behandling.bekreftArbeidIFlereLandOgFortsett();

    // Steg 4: Velg type arbeid
    await behandling.velgLønnetArbeidIToEllerFlereOgFortsett();

    // Steg 5: Velg prosentandel
    await behandling.velgProsentandelOgFortsett();

    // Steg 6: Vedtak
    await behandling.fyllUtVedtakOgFatt('Lo', 'da');

    console.log('✅ EU/EØS "Arbeid i flere land" arbeidsflyt fullført steg-for-steg');
  });
});
